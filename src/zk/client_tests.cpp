...
GTEST_TEST_F (client_tests, create) {
  ...
  auto name = std::move(f_create).get().name();
...
GTEST_TEST_F (client_tests, watch_change) {
  ...
  auto ev = std::move(watch).next().get();
...
GTEST_TEST_F (client_tests, watch_children) {
  ...
  auto ev = std::move(watch_child_creation).next().get();
  ...
  c.erase(root_name + "/a");
  auto ev2 = std::move(watch_child_erase).next().get();
...
GTEST_TEST_F (client_tests, watch_exists) {
  ...
  auto ev = std::move(watch_creation).next().get();
  ...
  auto ev2 = std::move(watch_erase).next().get();
